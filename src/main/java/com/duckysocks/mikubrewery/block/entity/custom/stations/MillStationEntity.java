...
public class MillStationEntity extends BlockEntity implements ImplementedInventory, ExtendedScreenHandlerFactory<BlockPos> {
    private final DefaultedList<ItemStack> inventory = DefaultedList.ofSize(1, ItemStack.EMPTY);
    private float rotation = 0;

    public MillStationEntity(BlockPos pos, BlockState state) {
    ...
    public Text getDisplayName() {
        return Text.literal("Mill");
    }

    @Override
    public @Nullable ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
        return new MillStationScreenHandler(syncId, playerInventory, this.pos);
    }
